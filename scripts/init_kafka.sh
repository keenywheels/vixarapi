...
do
  ...
  echo '{"site":"https://vixar.tech/","msg":"MESSAGE:'"${i}"', Мы предлагаем долговечную и качественную продукцию по самым выгодным ценам. Подробная информация о продукции: 1. Сверхвысокая совместимость с чипами: T76 не только поддерживает распространенные на рынке чипы EEPROM, EPROM и FLASH, но также имеет надежную поддержку микроконтроллеров и программируемых логических устройств (GAL/CPLD) с возможностью настройки и добавления новых моделей чипов. Независимо от того, имеете ли вы дело с последовательными, параллельными ROM, SPI NAND или даже чипами EMMC/EMCP, T76 с легкостью с ними справляется.","date":"2025-10-26 16:45:47"}' | kafka-console-producer \
